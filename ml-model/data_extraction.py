    ...
    for i, article in enumerate(articles):
        if article:
            try:
                ...
                analysis = analysis_pipeline(prompt, max_length=200, num_return_sequences=1)[0]['generated_text']
...
def preprocess_data():
    ...
    data['sentiment'] = [s['label'] for s in get_sentiments(data['article_text'].tolist())]
    data['sentiment_score'] = [s['score'] for s in get_sentiments(data['article_text'].tolist())]
    ...
    print("Preprocessing complete. Data saved to 'preprocessed_data.csv' and overview saved to 'data_overview.txt'.")
